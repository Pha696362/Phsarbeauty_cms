...
@Component({
  selector: 'app-statistic-period',
  templateUrl: './statistic-period.component.html',
  styleUrls: ['./statistic-period.component.scss']
})
export class StatisticPeriodComponent implements OnInit {
  @Input() fromDate: Date;
  @Input() toDate: Date;
  ...
  changePeriod(event) {
    ...
    console.log(period)
    if (value.key === 5) {
      this.dayOfPeriod.enable();
      this.monthOfPeriod.enable();
      this.yearOfPeriod.enable();
      this.toDayOfPeriod.enable();
      this.toMonthOfPeriod.enable();
      this.toYearOfPeriod.enable();
    }
  }
