...
@Component({
  selector: 'app-suspect-arrested',
  templateUrl: './suspect-arrested.component.html',
  styleUrls: ['./suspect-arrested.component.scss']
})
export class SuspectArrestedComponent implements OnInit {
  tabs = tabs.suspectArrested;
  labels = personLabels;
  constructor(
  ...
  ngOnInit() {
    this.store.fetchData(personTypeObj.suspectArrested.key);
  }

  create() {
    let dialogRef = this.dialog.open(AddPersonComponent, {
      data: { type: personTypeObj.suspectArrested },
      width: '35vw',
      height: '100vh',
      role: 'dialog',
    });
