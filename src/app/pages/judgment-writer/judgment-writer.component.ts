...
@Component({
  selector: 'app-judgment-writer',
  templateUrl: './judgment-writer.component.html',
  styleUrls: ['./judgment-writer.component.scss']
})
export class JudgmentWriterComponent implements OnInit {
  tabs = tabs.judgmentWriter;
  labels = personLabels;
  constructor(
  ...
  ngOnInit() {
    this.store.fetchData(personTypeObj.judgmentWriter.key);
  }

  create() {
    let dialogRef = this.dialog.open(AddPersonComponent, {
      data: { type: personTypeObj.judgmentWriter },
      width: '35vw',
      height: '100vh',
      role: 'dialog',
    });
