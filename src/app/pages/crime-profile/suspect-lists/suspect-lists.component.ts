...
@Component({
  selector: 'app-suspect-lists',
  templateUrl: './suspect-lists.component.html',
  styleUrls: ['./suspect-lists.component.scss']
})
export class SuspectListsComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.parent.parent.params.subscribe(param1 => {
      this.store.fetchpersonCrime(param1.id, "suspects");
    })
  }

  create(item: any) {
    let dialogRef = this.dialog.open(AddVictimsComponent, {
      data: { item, type: personTypeObj.suspect },
      width: '35vw',
      height: '100vh',
      role: 'dialog',
    });
